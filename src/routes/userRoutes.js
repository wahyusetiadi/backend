...
router.post("/register", (req, res) => {
  ...
  const query =
    "INSERT INTO users (name, email, password, role, cabang) VALUES (?, ?, ?, ?, ?)";
  db.get(
    `
    ...
    (err, row) => {
      ...
      db.run(
        query,
        [name, email, hashedPassword, role, cabang],
        function (err) {
          if (err) {
            return res
              .status(500)
              .json({ message: "Gagal mendaftarkan user", error: err });
          }
          res
            .status(201)
...
router.post("/login", (req, res) => {
  ...
  const query =
    "SELECT name, password, role, cabang FROM users WHERE name = ? AND isDeleted = 0";
  db.get(query, [name], (err, user) => {
    ...
    const token = jwt.sign(
      { id: user.id, name: user.name, role: user.role, cabang: user.cabang },
      "jwt_token_secret",
      { expiresIn: "12h" }
    );
    ...
    fs.appendFile(
      path.join(__dirname, "..", "..", "data", "login_logs.txt"),
      logMessage,
      (err) => {
        if (err) {
...
router.get("/", (req, res) => {
  const query = `SELECT id, name, email, role, cabang, dibuat_tanggal, dibuat_jam, isDeleted FROM users WHERE isDeleted = 0`;
...
router.put("/recovery/:id", (req, res) => {
  ...
  const query = `UPDATE users SET isDeleted = 0 WHERE id = ?`;
  db.run(query, [id], function (err) {
    if (err) {
    ...
    res
      .status(200)
      .json({ message: `Data dengan ID: ${id} berhasil dipulihkan` });
  });
});
...
router.put("/:id", (req, res) => {
  ...
  if (!name || !email || !role || !cabang) {
    return res.status(400).json({
      message: "Semua field wajib diisi (name, email, role, cabang).",
    });
  }
  ...
  db.get(checkQuery, [email, id], (err, row) => {
    ...
    db.run(
      updateQuery,
      [name, email, updatedPassword || null, role, cabang, id],
      function (err) {
        if (err) {
          return res
            .status(500)
        ...
        res
          .status(200)
          .json({ message: `User dengan ID ${id} berhasil diperbarui.` });
      }
    );
  });
});
...
router.post("/logout", (req, res) => {
  ...
  jwt.verify(token, "jwt_token_secret", (err, decoded) => {
    ...
    fs.appendFile(
      path.join(__dirname, "..", "..", "data", "logout_logs.txt"),
      logMessage,
      (err) => {
        if (err) {
...
router.delete("/perm-delete/:id", (req, res) => {
  const { id } = req.params;
